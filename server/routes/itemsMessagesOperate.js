...
//var db_xchange = mongojs('mongodb://liang121:123@ds019766.mlab.com:19766/xchange',['item_messages']); 
//var db_xchange = mongojs('xchange',['item_messages']);
...
router.post('/',function(req,res){
    ...
    function updateUnread(idArray){
        db_xchange.item_messages.update(
            {_id:{$in:idArray}},
            {$set:{isReaded:false}},
            {multi: true}
            ,function(err,doc){
    ...
    function updateDelete(idArray){
        db_xchange.item_messages.remove({_id:{$in:idArray}},function(err,doc){
            if(err){
                return res.send(err);
            }else{
