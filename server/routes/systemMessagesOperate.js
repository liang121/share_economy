...
//var db_xchange = mongojs('mongodb://liang121:123@ds019766.mlab.com:19766/xchange',['system_messages']); 
//var db_xchange = mongojs('xchange',['system_messages']);
...
router.post('/',function(req,res){
    ...
    function updateGet(){
        db_xchange.system_messages.find(
            function(err,doc){
                if(err){
                    return res.send(err);
                }else{
    ...
    function updateUnread(idArray){
        db_xchange.system_messages.update(
            {_id:{$in:idArray}},
            {$set:{isReaded:false}},
            {multi: true}
        ,function(err,doc){
    ...
    function updateDelete(idArray){
        db_xchange.system_messages.remove({_id:{$in:idArray}},function(err,doc){
            if(err){
                return res.send(err);
            }else{
